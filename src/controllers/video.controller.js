...
const updateVideo = asyncHandler(async (req, res) => {
  ...
  const { title, description, isPublished } = await Video.findById(videoId);
  const thumbnailLocalPath = req.file?.path;

  if (!title || !description || !isPublished) {
    throw new ApiError(400, "All three feilds are required");
  }
  if (!thumbnailLocalPath) {
  ...
  await Video.deleteImageFromCloudinary(oldThumbnail);
...
const deleteVideo = asyncHandler(async (req, res) => {
  ...
  if (!video) {
    throw new ApiError(404, "Vidoe not foundss");
  }
  const deletedVideo = await Video.findByIdAndDelete(video);

  if (!deletedVideo) {
    throw new ApiError(500, "Internal server error while delting the video");
  }
...
const togglePublishStatus = asyncHandler(async (req, res) => {
  ...
  if (!togglePublished) {
    throw new ApiError(
      500,
      "Internal Server error while toggling published status"
    );
  }
  return res
    .status(200)
    .json(new ApiResponse(200, togglePublished, "Status toggled successfully"));
});
